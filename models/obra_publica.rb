...
class ObraPublica
  ...
  def initialize(id, nombre, etapa, tipo, area_responsable, descripcion, monto_contrato, 
    ...
    raise InputException.new('La descripción no puede contener más de 2000 caracteres') unless descripcion.lenght <= 2000
    ...
    @obras = @parser.parse
    if etapa = 'Finalizada' then raise InputException.new('Toda obra FINALIZADA debe tener un porcentaje de avance del 100% y una fecha de finalización real acorde') 
      unless (porcentaje_avance == 100 && ! fecha_fin_real.empty?)
    end
    if porcentaje_avance == 100 then raise InputException.new('El 100% de avance sólo puede ser utilizado en obras FINALIZADAS') 
      unless etapa == 'Finalizada'
    end
    if ! fecha_fin_real.empty? then raise InputException.new('La fecha de finalización real sólo está disponible para obras FINALIZADAS') 
      unless etapa == 'Finalizada'
    end
  end
